  ...
  isSame (eArray) {
    ...
    for (var i = 0; i < eArray.length; i++) {
      ...
      if (idx >= 0) {
        ...
        if (se.title && e.title && se.title !== e.title) {
          return 0
        }
        if (se.startDate && e.startDate && se.startDate !== e.startDate) {
          return 0
        }
        if (se.endDate && e.endDate && se.endDate !== e.endDate) {
          return 0
        }
        if (se.location && e.location && se.location !== e.location) {
          return 0
        }
        if (se.geo && e.geo) {
          if (se.geo.lat !== e.geo.lat || se.geo.lon !== e.geo.lon) {
            return 0
          }
        }
        if (se.description && e.description && se.description !== e.description) {
          return 0
        }
      } else {
        return 0
